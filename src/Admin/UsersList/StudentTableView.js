...
const CommonUserProfile = (props) => {
  ...
  // const TeamId = studentType?.type === 0 ? studentId?.student_id : studentType?.type;
  const TeamId = Number(studentType) === 0 ? studentId : studentType;
  useEffect(() => {
    submittedApi();
  }, []);
  ...
  const mentorViewApi = () => {
    ...
    if (typeof TeamId !== "string") {
      supId = encryptGlobal(JSON.stringify(TeamId));
    } else {
      supId = encryptGlobal(TeamId);
    }
    var config = {
      method: "get",
      ...
      <div className="content">
        <Row className="d-flex align-items-center justify-content-between mb-2">
          <div className="col-auto">
            <h4 className="mt-2">
              Student Profile -{" "}
              <span style={{ color: "#17A2B8" }}>{data?.full_name} </span>{" "}
              <span style={{ color: "#ffa800" }}>
                {data?.type === 0 ? " (Pilot Member)" : " (Crew Member)"}
              </span>
            </h4>
          </div>
              ...
              <Table striped bordered>
                <tbody>
                  ...
                  <tr>
                    ...
                    <td className="w-50">
                      {data?.college_town ? data?.college_town : "-"}
                    </td>
                  </tr>
                  <tr>
                    <td>
